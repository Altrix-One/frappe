...
class TestCustomFunctionsMariaDB(AltrixTestCase):
...
class TestCustomFunctionsPostgres(AltrixTestCase):
	def test_concat(self):
...
class TestParameterization(AltrixTestCase):
...
@run_only_if(db_type_is.MARIADB)
class TestBuilderMaria(AltrixTestCase, TestBuilderBase):
...
@run_only_if(db_type_is.POSTGRES)
class TestBuilderPostgres(AltrixTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual('SELECT * FROM "tabNotes"', frappe.qb.from_("Notes").select("*").get_sql())
		self.assertEqual('SELECT * FROM "__Auth"', frappe.qb.from_("__Auth").select("*").get_sql())
